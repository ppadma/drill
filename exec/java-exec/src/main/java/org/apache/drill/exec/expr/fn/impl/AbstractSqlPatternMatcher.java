...
// To get good performance for most commonly used patterns
// i.e. CONSTANT('ABC'), STARTSWITH('%ABC'), ENDSWITH('ABC%') and CONTAINS('%ABC%'),
// we have simple pattern matchers.  Anything that is not simple is considered
...
// We do byte comparison against native memory. Overall, this approach
...
public abstract class AbstractSqlPatternMatcher implements SqlPatternMatcher {
  ...
  final ByteBuffer patternByteBuffer;
  ...
  public AbstractSqlPatternMatcher(String patternString) {
    ...
    try {
      ...
      throw UserException.validationError(e)
          .message("Failure to decode pattern %s using UTF-8", patternString)
  ...
  public abstract int match(int start, int end, DrillBuf drillBuf);
}
